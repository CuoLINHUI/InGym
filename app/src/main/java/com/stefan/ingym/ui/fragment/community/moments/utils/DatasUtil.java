package com.stefan.ingym.ui.fragment.community.moments.utils;
...
public class DatasUtil {
	...
	public static List<MomentItem> createCircleDatas() {
		List<MomentItem> circleDatas = new ArrayList<MomentItem>();
		for (int i = 0; i < 15; i++) {
			...
			if (type == 0) {
				...
				item.setType("2");	// 表示发表的动态是图片
				item.setPhotos(createPhotos());
			}else {
				item.setType("3");	// 表示发表的动态是视频
	...
	public static List<LikeItem> createFavortItemList() {
		int size = getRandomNum(users.size());
		List<LikeItem> items = new ArrayList<LikeItem>();
		List<String> history = new ArrayList<String>();
		if (size > 0) {
			for (int i = 0; i < size; i++) {
				LikeItem newItem = createFavortItem();
	...
	public static LikeItem createFavortItem() {
	...
	public static LikeItem createCurUserFavortItem() {
		LikeItem item = new LikeItem();
		item.setId(String.valueOf(favortId++));
	...
	public static MomentItem createVideoItem(String videoUrl, String imgUrl){
		MomentItem item = new MomentItem();
		item.setId(String.valueOf(circleId++));
		item.setUser(curUser);
		//item.setContent(getContent());
